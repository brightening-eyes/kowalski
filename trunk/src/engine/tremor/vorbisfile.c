...
static void _add_serialno(ogg_page *og,ogg_uint32_t **serialno_list, int *n){
  ...
  if(*serialno_list){
    *serialno_list = (ogg_uint32_t*)_ogg_realloc(*serialno_list, sizeof(**serialno_list)*(*n));
  }else{
    *serialno_list = (ogg_uint32_t*)_ogg_malloc(sizeof(**serialno_list));
  }
...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  if((ret=_fetch_headers(vf,vf->vi,vf->vc,&serialno_list,&serialno_list_size,NULL))<0){
    ...
    vf->serialnos=(ogg_uint32_t*)_ogg_calloc(serialno_list_size+2,sizeof(*vf->serialnos));
